...
class ShippingProcessorTest {
    ...
    @BeforeEach
    void setUp() {
        processor = new ShippingProcessor(sqsClient, OBJECT_MAPPER, orderService, messagingResources);
    }

    @Test
    @DisplayName("poll() marks an order SHIPPING and deletes the message")
    void poll_marksOrderShippingAndDeletesMessage() {
        when(messagingResources.shippingQueueUrl()).thenReturn("http://queue");
    ...
    void poll_noopWhenQueueUnavailable() {
        when(messagingResources.shippingQueueUrl()).thenReturn(null);
